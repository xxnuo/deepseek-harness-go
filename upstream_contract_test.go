...
func TestUpstreamContractRPCMethods(t *testing.T) {
	...
	got := dispatchMethods(t, filepath.Join(root, "rpc.go"))
...
func TestUpstreamContractTypertInvocations(t *testing.T) {
	...
	assertStringSet(t, "Typert Remote descriptors", remoteDescriptorRows(t, filepath.Join(root, "remote.go")), typertInvocationRows(t, paths))
}
