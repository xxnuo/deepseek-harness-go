...
func TestOriginalFrontendBootsEveryPluginAsset(t *testing.T) {
	...
	if len(graph.Entries) != 56 {
		t.Fatalf("boot graph has %d entries, want 56", len(graph.Entries))
	}
...
func TestNewUsesEmbeddedRuntimeAssetsByDefault(t *testing.T) {
	...
	if len(graph.Entries) != 56 {
		t.Fatalf("default embedded boot graph has %d entries, want 56", len(graph.Entries))
	}
}
