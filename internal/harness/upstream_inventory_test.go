...
func TestUpstreamContractWorkspaceInventory(t *testing.T) {
	...
	inventoryPath := filepath.Join(repository, "upstream_inventory.tsv")
	rows := readUpstreamInventory(t, inventoryPath)
...
func writeUpstreamInventory(t *testing.T, path string, rows []upstreamInventoryRow, owned map[string][]upstreamWorkspaceFile) {
	...
	output.WriteString("# status\tworkspace root\ttracked files\tpath+blob sha256\tGo surface\n")
	output.WriteString("# ported: runtime behavior and applicable wiring exist; partial: named behavior or applicable wiring remains incomplete.\n")
	for _, row := range rows {
		files, digest, goSurface := "-", "-", "-"
		if row.status != "frontend" {
			files = strconv.Itoa(len(owned[row.root]))
			digest = upstreamFileContentDigest(owned[row.root])
			if len(row.goFiles) > 0 {
				goSurface = strings.Join(row.goFiles, ",")
			}
		}
		fmt.Fprintf(&output, "%s\t%s\t%s\t%s\t%s\n", row.status, row.root, files, digest, goSurface)
	}
