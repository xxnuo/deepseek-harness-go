...
func TestDeepSeekPluginPackageInventoryResolvesMaterializedPresetPackages(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(packageDir, "package.json"), []byte(`{"name":"@deepseek-ai/dsh-fixture","version":"0.1.2-alpha.1"}`), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	want := []DeepSeekPluginPackageIdentity{
		{Name: "@deepseek-ai/dsh-fixture", Version: "0.1.2-alpha.1"},
