...
func TestCredentialsPersistWithOwnerOnlyPermissionsAndEvents(t *testing.T) {
	...
	if err != nil || !strings.Contains(string(data), "version: 1") || !strings.Contains(string(data), "refs:") || !strings.Contains(string(data), "secret-value") {
...
func TestUpstreamYAMLSettingsAndCredentialsReload(t *testing.T) {
	...
	if !strings.Contains(string(credentialData), "version: 1") || !strings.Contains(string(credentialData), "refs:") || !strings.Contains(string(credentialData), "DEEPSEEK_API_KEY: yaml-secret") {
...
func TestVersionedCredentialsYAMLLoadsAndPreservesRecords(t *testing.T) {
	const firstRef = "DSH_VERSIONED_CREDENTIAL_FIRST"
	const secondRef = "DSH_VERSIONED_CREDENTIAL_SECOND"
	t.Setenv(firstRef, "")
	t.Setenv(secondRef, "")
	dir := t.TempDir()
	path := filepath.Join(dir, credentialsYAMLName)
	if err := os.WriteFile(path, []byte("version: 1\nrefs:\n  "+firstRef+": first-value\nrecords:\n  llm-pi-ai/example:\n    kind: grant\n    payload:\n      type: oauth\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	cfg := DefaultConfig()
	cfg.SessionTitleLLM.Enabled = false
	cfg.DataDir, cfg.Workspace, cfg.Provider, cfg.Model, cfg.APIKey, cfg.Persist = dir, dir, "echo", "echo", "", true
	e, err := New(WithConfig(cfg))
	if err != nil {
		t.Fatal(err)
	}
	configured, source, writable := e.credentialInfo(firstRef)
	if !configured || source != "file" || !writable {
		t.Fatalf("versioned credential = %v/%s/%v", configured, source, writable)
	}
	if rpcErr := e.setCredential(secondRef, "second-value"); rpcErr != nil {
		t.Fatal(rpcErr)
	}
	if err := e.Close(); err != nil {
		t.Fatal(err)
	...
	text := string(data)
	for _, fragment := range []string{"version: 1", "refs:", firstRef + ": first-value", secondRef + ": second-value", "records:", "llm-pi-ai/example:", "kind: grant"} {
		if !strings.Contains(text, fragment) {
			t.Fatalf("credential document omitted %q: %q", fragment, text)
		}
	}
	e2, err := New(WithConfig(cfg))
	if err != nil {
	...
	t.Cleanup(func() { _ = e2.Close() })
	configured, source, writable = e2.credentialInfo(secondRef)
	if !configured || source != "file" || !writable {
		t.Fatalf("reloaded versioned credential = %v/%s/%v", configured, source, writable)
	}
}
...
func TestSettingsAndCredentialRemoteEvents(t *testing.T) {
	...
	want := map[string]bool{"settings/document-updated": false, "credentials/reference-updated": false}
	deadline := time.After(2 * time.Second)
	for !want["settings/document-updated"] || !want["credentials/reference-updated"] {
		select {
		case frame := <-host:
...
func TestUnsetCredentialReloadsCurrentDiskValue(t *testing.T) {
	...
	stale.credentialService.watch()
	stale.credentialService.watchWG.Wait()
	stale.credentialService.watch = nil
