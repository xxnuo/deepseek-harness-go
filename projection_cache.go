...
const sessionProjectionCacheVersion = 3
...
func decodeProjectionCacheRecord(value any) (projectionCacheRecord, error) {
	...
	if record.Identity.CreatedAt < 0 || record.Seq < -1 || record.Values == nil {
...
func (medium *projectionCacheMedium) snapshot(header SessionHeader, lastSeq int, exact bool) (ProjectionSnapshot, bool) {
	medium.mu.RLock()
	record, ok := medium.records[header.ID]
	medium.mu.RUnlock()
	if !ok || !sameProjectionIdentity(record.Identity, projectionIdentity(header)) || record.Seq > lastSeq || exact && record.Seq != lastSeq {
...
func newSessionProjectionCache(root string, config SessionProjectionCacheConfig) (*sessionProjectionCache, error) {
	medium, err := acquireProjectionCacheMedium(root)
	if err != nil {
		return nil, err
	}
	return &sessionProjectionCache{config: config, medium: medium, dirty: map[*Session]*projectionCacheDirty{}}, nil
}
...
func (cache *sessionProjectionCache) writeSession(ctx context.Context, session *Session) error {
	...
	record := projectionCacheRecord{
		Identity: projectionIdentity(session.Header), Seq: len(session.Events) - 1,
		Values: sessionProjectionValues(session.Events, session.Title),
	}
...
func (cache *sessionProjectionCache) putSnapshot(ctx context.Context, header SessionHeader, snapshot ProjectionSnapshot) error {
	return cache.medium.put(ctx, header.ID, projectionCacheRecord{
		Identity: projectionIdentity(header), Seq: snapshot.AsOfSeq, Values: snapshot.Values,
	})
}
...
func (e *Engine) SessionProjectionSnapshot(ctx context.Context, id string) (ProjectionSnapshot, error) {
	...
	if e.projectionCache != nil {
		if cached, ok := e.projectionCache.medium.snapshot(header, lastSeq, true); ok {
			session.mu.Unlock()
			return cached, nil
		}
	}
	snapshot := ProjectionSnapshot{AsOfSeq: lastSeq, Values: sessionProjectionValues(session.Events, session.Title)}
	session.mu.Unlock()
	if e.projectionCache != nil {
		if err := e.projectionCache.putSnapshot(ctx, header, snapshot); err != nil {
