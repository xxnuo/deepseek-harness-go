...
func (p *bedrockProvider) Complete(ctx context.Context, req ChatRequest, onDelta func(Delta) error) (Completion, error) {
	...
	region := bedrockRegion(model, p.baseURL)
	endpoint, err := bedrockEndpoint(p.baseURL, region, model)
	if err != nil {
	...
	if os.Getenv("AWS_BEDROCK_SKIP_AUTH") != "1" {
		bearer := firstNonBlank(p.apiKey, os.Getenv("AWS_BEARER_TOKEN_BEDROCK"))
		if bearer != "" {
			hreq.Header.Set("Authorization", "Bearer "+bearer)
		} else {
			credential, credErr := resolveAWSCredential()
			if credErr != nil {
...
func bedrockRegion(model, baseURL string) string {
	...
	if region := firstNonBlank(os.Getenv("AWS_REGION"), os.Getenv("AWS_DEFAULT_REGION")); region != "" {
		return region
	}
...
func bedrockEndpoint(baseURL, region, model string) (string, error) {
	base := firstNonBlank(baseURL, "https://bedrock-runtime."+region+".amazonaws.com")
	if parsed, err := url.Parse(base); err == nil && strings.HasPrefix(parsed.Hostname(), "bedrock-runtime.us-east-1.") && region != "us-east-1" && firstNonBlank(os.Getenv("AWS_REGION"), os.Getenv("AWS_DEFAULT_REGION")) != "" {
...
func resolveAWSCredential() (awsCredential, error) {
	credential := awsCredential{
		AccessKeyID:     firstNonBlank(os.Getenv("AWS_ACCESS_KEY_ID"), os.Getenv("AWS_ACCESS_KEY")),
		SecretAccessKey: firstNonBlank(os.Getenv("AWS_SECRET_ACCESS_KEY"), os.Getenv("AWS_SECRET_KEY")),
	...
	profile := firstNonBlank(os.Getenv("AWS_PROFILE"), "default")
