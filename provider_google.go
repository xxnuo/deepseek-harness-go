...
func (p *googleProvider) endpointAndAuth(ctx context.Context, model string) (string, string, string, error) {
	...
	project := firstNonBlank(os.Getenv("GOOGLE_CLOUD_PROJECT"), os.Getenv("GCLOUD_PROJECT"))
	...
	key := strings.TrimSpace(p.apiKey)
	if key == "" {
		key = strings.TrimSpace(os.Getenv("GOOGLE_CLOUD_API_KEY"))
	}
	if key != "" && !strings.HasPrefix(key, "<") {
		return endpoint, "", key, nil
	}
	token, err := googleADCAccessToken(ctx, p.client)
	if err != nil {
		return "", "", "", err
	}
...
func googleADCAccessToken(ctx context.Context, client *http.Client) (string, error) {
	...
	host := firstNonBlank(os.Getenv("GCE_METADATA_HOST"), "metadata.google.internal")
