...
func TestStaticOriginalFrontendFallbackAndBoot(t *testing.T) {
	...
	defer server.Close()
	response, err := server.Client().Get(server.URL + "/nested/route")
	if err != nil {
		t.Fatal(err)
	}
	defer response.Body.Close()
	body := readAll(t, response)
	if response.StatusCode != http.StatusOK || !strings.Contains(body, "original UI") || !strings.Contains(body, "window.__DSH_BOOT__") {
		t.Fatalf("SPA fallback = %d %q", response.StatusCode, body)
	}
...
func TestOriginalFrontendBootsEveryPluginAsset(t *testing.T) {
	e := newIntegrationEngine(t)
	assets, err := MaterializeAssets(t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
