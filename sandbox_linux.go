...
func sandboxInvocation(mode, workspace, workdir, program string, args []string) (string, []string, error) {
	...
	switch runner.kind {
	case "bwrap":
		profile := []string{"--ro-bind", "/", "/", "--dev", "/dev", "--proc", "/proc", "--die-with-parent"}
...
func probeLinuxSandboxRunner() linuxSandboxRunner {
	if path, err := exec.LookPath("bwrap"); err == nil && probeSandboxCommand(path, []string{"--ro-bind", "/", "/", "--dev", "/dev", "--proc", "/proc", "--die-with-parent", "--", "true"}) {
