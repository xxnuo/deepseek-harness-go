...
func (e *Engine) staticHandler(w http.ResponseWriter, r *http.Request) {
	...
	if st, err := os.Stat(path); err == nil && !st.IsDir() && path != index {
	...
	if data, err := os.ReadFile(index); err == nil {
		html := injectBoot(string(data), e)
		html = injectThemeBootstrap(html, e)
...
func injectBoot(html string, e *Engine) string {
	graph, _, err := e.buildBootGraph()
	if err != nil {
		graph = BootGraph{Rev: shortRevision("[]"), Entries: []BootEntry{}}
	}
...
func injectThemeBootstrap(html string, e *Engine) string {
  ...
  document.body.toggleAttribute('data-ds-dark-theme', dark)
	...
	body := regexp.MustCompile(`(?i)<body(?:\s[^>]*)?>`).FindStringIndex(html)
	if body == nil {
		return html + script
	}
	return html[:body[1]] + script + html[body[1]:]
}
func serveFallback(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	_, _ = io.WriteString(w, `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>DeepSeek Harness</title></head><body><main><h1>DeepSeek Harness</h1><p>Go backend is running.</p></main></body></html>`)
}
