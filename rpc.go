...
func (e *Engine) dispatch(ctx context.Context, method string, raw json.RawMessage, rpcIDs ...string) (any, *RPCError) {
	...
	case "session.create":
		...
		if ws, _ := p["workspaceId"].(string); ws != "" {
			e.mu.RLock()
			w := e.workspaces[ws]
			e.mu.RUnlock()
			if w == nil {
				return nil, rpcError("workspace-not-found", "workspace not found", map[string]any{"workspaceId": ws})
			}
			cwd = w.Path
		}
		...
		if ws, _ := p["workspaceId"].(string); ws != "" {
			if attachErr := e.AttachSession(ws, sid); attachErr != nil {
				return nil, rpcError("workspace-attach-failed", attachErr.Error(), map[string]any{"sessionId": sid, "workspaceId": ws})
			}
		}
		out := map[string]any{"sessionId": sid}
		if preset != "" {
			out["agentPreset"] = preset
		}
		return out, nil
	case "session.history":
		...
		if before < 0 {
			s, _ := e.getSession(id)
			asOf := -1
			projections := sessionProjectionValues(nil, "")
			if s != nil {
				s.mu.Lock()
				asOf = len(s.Events) - 1
				projections = sessionProjectionValues(s.Events, s.Title)
				s.mu.Unlock()
			}
			value["projections"] = map[string]any{"asOfSeq": asOf, "values": projections}
		}
		return value, nil
	case "session.models":
		id, _ := p["sessionId"].(string)
